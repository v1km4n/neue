	...
	execute(message, args) {
		...
		steam.resolve(args[0]).then(steamID => {
			...
			if (etf2lPlayer.status.code == 200) {
				...
				let ETF2lCountry = etf2lPlayer.player.country;
				...
				if (HLTeamNo != null) {
					...
					if (listofHLTeamSeasons !== null) { //team exists, but haven't played any matches yet (HLTeamDiv stays null)
				...
				if (SixesTeamNo != null) {
					...
					if (listofSixesTeamSeasons !== null) { //team exists, but haven't played any matches yet (SixesTeamDiv stays null)
				...
				var ETF2LEmbed = new Discord.MessageEmbed()
					...
					.setColor('#0099ff')
					.setTitle(`ETF2L: ${ETF2LNickName} (${ETF2lCountry})`)
